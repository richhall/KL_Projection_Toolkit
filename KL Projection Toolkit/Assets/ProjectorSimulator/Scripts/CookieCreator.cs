...
public class Cookie
{
    ...
    RenderTexture imageWithBorder;
    RenderTexture imageShifted;
    ...
    // shaders which strip the individual channels, offloading to gpu YAY
    static Material stripRed, stripGreen, stripBlue;
    ...
    void CreateTexture()
    {
        ...
        else
        {
            ...
            imageShifted = new CustomRenderTexture(textureSize, textureSize, RenderTextureFormat.ARGB32);
            imageShifted.wrapMode = TextureWrapMode.Clamp;
        }
    }
    ...
    void Initialise()
    {
        ...
        if (stripRed == null)
        {
            stripRed   = new Material(Shader.Find("Hidden/StripChannel_Red"));
            stripGreen = new Material(Shader.Find("Hidden/StripChannel_Green"));
            stripBlue  = new Material(Shader.Find("Hidden/StripChannel_Blue"));
        }
        ...
        if (!pixelByPixel)
        {
            ...
            imageWithBorder = new RenderTexture(projectedImage.width + 2 * blackBorderSize, projectedImage.height + 2 * blackBorderSize,0, RenderTextureFormat.ARGB32);
            imageWithBorder.wrapMode = TextureWrapMode.Clamp;
        }
